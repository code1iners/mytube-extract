...
- 기존 `ready` 요청 화면의 health 백그라운드 갱신은 제목 옆 낮은 강조도의 표시와 `aria-busy`만 갱신하며, health 상태 live region은 실제 상태 전이 또는 장애 응답에서만 알린다.
...
- 숨김 polite live region은 동일 polling tick이 아니라 상태 값 전이와 삭제만 알린다. 기존 `ready` form의 health 백그라운드 갱신은 `aria-live="off"`로 반복 공지를 막는다.
