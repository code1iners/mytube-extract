...
- 요청 설정 흐름 다음 보조 영역에서 API 연결과 worker 준비 상태를 확인 중·준비됨·worker 중단·확인 실패의 텍스트와 아이콘으로 표시한다. 확인 중·준비됨은 compact status로, API 확인 실패·worker 미가용은 구분된 확장 안내로 표시한다. 정상 상태의 마지막 확인 시각은 보조 메타로만 제공하고 `다시 확인` 동작을 유지하며, 확인 중에는 재확인 요청을 중복 전송하지 않는다.
- health가 준비되지 않았을 때 제출 button은 readiness live 설명을 `aria-describedby`로 참조하고 CTA 아래에 같은 health 차단 문구를 반복하지 않는다. 파일 validation 안내는 readiness가 정상일 때만 제출 button 가까이에 표시한다.
