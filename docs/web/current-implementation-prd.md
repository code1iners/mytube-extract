...
- 요청 전 `GET /health`로 worker 가능 여부를 확인한다. `checking`·`failed`·`unavailable`에서는 상태 우선 화면을 먼저 보여주고 `ready`에서만 요청 form을 표시한다. 백그라운드 확인으로 form이 숨겨져도 입력값과 선택 파일은 보존한다.
...
- 요청·처리·완료·요청 내역은 API 상태에 대응하는 `원본 → 추출 → 파일 수령` 흐름을 표시하며, `/video`는 `U`, `/subtitles`는 `F` 단축키로 첫 입력 동작에 focus할 수 있다. 단축키는 text editing target과 modifier 조합에서는 작동하지 않는다.
- 자막 처리 방식의 `base.en`, `small.en`, `Whisper`, `worker` 설명은 기본으로 접힌 native disclosure에서만 제공하며 사용자 결과 중심 문구와 속도·정확도 선택은 계속 바로 보인다.
