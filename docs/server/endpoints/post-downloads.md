...
- side effect: 입력 URL에서 검증한 `videoId`로 query-free canonical YouTube watch URL을 만들어 `ExtractionJob`에 저장한다. 클라이언트가 보낸 `title`은 사용하지 않는다. 같은 `videoId`/`type`/`quality`의 유효한 R2 asset이 있으면 asset 제목을 제목 없는 새 job에 복사해 즉시 `completed`로 만든다. 이미 제목이 있는 요청은 asset 제목으로 덮어쓰지 않는다.
