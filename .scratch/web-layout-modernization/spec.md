...
요청 전에 표시하는 API (Application Programming Interface: 앱과 서버가 통신하는 규약)·worker readiness는 사용자가 입력을 시작하는 데 필요한 조건이다. 최초 확인 중에는 폼을 숨긴 status-first 화면이 상태와 복구 동작을 보여주고, 준비됨이 확인되면 입력을 연다. 이미 준비된 폼은 백그라운드 health 확인 중에도 유지하며, 완료된 응답이 실패·미가용일 때만 다시 상태 화면으로 전환한다. 모바일 자막 요청은 파일 선택 영역과 두 처리 방식이 CTA (Call to Action: 주요 실행 버튼) 앞에 펼쳐져도 핵심 행동까지의 거리가 짧도록 밀도를 유지한다.
...
영상 추출은 `YouTube URL → 형식 → 품질 → 요청`, 자막 추출은 `로컬 영상 → 처리 방식 → 영어 SRT 생성` 순서가 한눈에 읽히도록 서로 다른 작업 구조를 갖는다. 두 route의 navigation과 page H2는 각각 `영상 추출`과 `자막 추출`로 통일하고 CTA는 구체적인 동작명을 유지한다. 최초 health 응답이 없을 때만 상태 우선 화면이 form을 대체하며, 이미 `ready`인 입력은 백그라운드 확인 중 보존한다. 완료된 응답의 API 확인 실패와 worker 미가용은 확장 안내에서 구분하고 같은 차단 이유를 중복 설명하지 않는다. 서버 job 생성 전 `요청 취소`는 브라우저 요청을 중단하지만, 생성 경쟁이 확인되면 접수증을 보존하고 서버 job이 취소됐다고 말하지 않는다.
...
- **readiness 표현 상태**: 최초 응답이 없는 `checking`에서만 status-first readiness panel이 form을 대체한다. 이미 `ready`인 form은 background refetch 중 유지하고, 완료된 health 응답이 `failed` 또는 `unavailable`이면 form을 대체한다. `failed`는 API 상태 요청 자체를 확인하지 못한 상태, `unavailable`은 API가 응답했지만 worker가 작업을 받을 수 없는 상태로 구분해 안내하며 모든 차단 상태에는 재확인 동작을 유지한다.
- **readiness 정보 우선순위**: 마지막 확인 시각은 정상 상태의 주 콘텐츠가 아니며 제목 근처 보조 메타로 낮춘다. 백그라운드 갱신은 제목 옆 작은 표시와 `aria-busy`로 알리고 `aria-live="off"`로 반복 공지를 막는다. 상태가 장애로 전환되어 form이 대체되어도 hook state의 URL·파일·선택값은 보존하며 입력 보존 안내를 한 번 표시한다. 제출 불가 이유는 readiness 영역의 canonical 안내 하나로 유지하고 CTA 인접 영역에서 긴 문구를 반복하지 않는다.
...
- **readiness 접근성**: 기존 `status`·`alert`, `aria-live`, `aria-busy`, 재확인 accessible name 계약을 유지한다. 장애 상태는 Nintendo red primary 재확인을 full-width로 먼저 놓고 기술 상세를 그 아래에 둔다. compact 표현은 시각적 축약일 뿐 상태 의미나 보조 기술 공지를 제거하지 않는다.
...
- **공통 흐름과 탐색**: request·processing·completed·history 항목은 실제 API 상태에 맞춰 번호 marker가 있는 `원본 → 추출 → 파일 수령`의 current step을 표시한다. active navigation label은 테마 본문색, red는 indicator/icon에만 사용하며 `/settings`는 primary 3탭 밖의 보조 route로 유지한다. 모바일에서는 `사용 안내`와 `설정`을 한 줄의 `더보기` 메뉴로 묶고 keyboard/pointer 닫기와 focus return을 제공한다.
...
- **URL 지우기와 터치 영역**: URL 지우기 control은 값이 있을 때만 렌더링하며 영상·자막의 파일 제거와 짧은 `지우기` 용어를 공유한다. 설정 링크와 지우기 control의 실제 clickable box는 너비와 높이 모두 최소 44px이며, 보이는 텍스트·accessible name은 기존 사용 목적을 유지한다.
...
- **빈 내역 문구**: 영상·자막 두 진입점, 현재 브라우저에만 남는 이력, 완료 파일 7일 보관이라는 제품 사실을 관련 empty/history/completion 위치에서 짧게 맥락화한다. API 응답이 상태와 완료 receipt의 원본 파일명·결과 형식·보관 기간의 source of truth이며, 같은 의미를 반복하는 제목·설명은 줄이고 새로운 보관 정책이나 동기화 기능을 암시하지 않는다.
...
- **readiness 상태**: 최초 `checking`에서는 form이 보이지 않고 status-first 안내와 재확인 동작이 보이는지, `ready`에서는 form이 보이는지 검증한다. ready background checking 동안 form·URL·파일·선택값이 유지되고, 완료된 failed/unavailable 응답에서만 form이 대체되는지 확인한다. 상태 변화가 `status` 또는 `alert`와 `aria-busy`로 전달되고, background ready 상태는 `aria-live="off"`이며, 같은 차단 설명이 중복 출력되지 않는지 확인한다.
...
- 근거는 `apps/web` PRODUCT 계약, 공통 DESIGN 계약, 2026-09-03 `apps/web` Impeccable critique(32/40), 현재 route·컴포넌트 구현, 기존 Web unit/browser 테스트다.
...
- 2026-09-03 critique 후속은 이 스펙의 이슈 06에서 C01–C10과 D01–D04로 추적한다. 구현·문서 동기화·자동 검증·bounded visual pass를 이 작업의 gate로 삼으며, 실제 API·worker/provider·물리 기기·화면 낭독기·production 검증은 별도 경계로 남긴다.
- 2026-09-03 최종 검증에서 lint·unit(17개 파일·124개)·build·browser smoke와 변경 소스 detector(`[]`)가 통과했다. browser smoke는 fixture API와 로컬 Chromium 증거이며 실제 API·worker/provider/R2·물리 safe-area·screen reader·production 검증은 완료로 주장하지 않는다.
