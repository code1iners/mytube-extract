...
요청 전에 표시하는 API (Application Programming Interface: 앱과 서버가 통신하는 규약)·worker readiness는 사용자가 입력을 시작하는 데 필요한 조건이지만, 현재는 폼보다 먼저 큰 카드로 나타난다. 특히 실패 상태는 화면의 가장 강한 콘텐츠가 되면서도 API 연결 실패와 worker 미가용을 구분해 설명하지 못한다. 모바일 자막 요청은 readiness, 220px 파일 선택 영역, 두 처리 방식이 모두 CTA (Call to Action: 주요 실행 버튼) 앞에 펼쳐져 핵심 행동까지의 시각적 거리가 길다.
...
기존 Nintendo 미니멀 플랫 정체성과 기능·상태·route 계약은 유지하면서 Web의 표면 계층과 작업 리듬을 재구성한다. 페이지 전체를 다시 하나의 카드로 보이게 만드는 외곽 표면을 약화하고, 제목·설명·입력·CTA가 여백과 타이포그래피만으로도 하나의 작업 흐름으로 읽히게 한다. border와 surface 전환은 실제 상호작용 경계, 오류, 완료 결과처럼 구분이 필요한 곳에만 사용한다.

영상 추출은 `YouTube URL → 형식 → 품질 → 요청`, 자막 추출은 `로컬 영상 → 처리 방식 → 영어 SRT 생성` 순서가 한눈에 읽히도록 서로 다른 작업 구조를 갖는다. API·worker readiness는 정상·확인 중일 때 compact status로 축약하고, API 확인 실패와 worker 미가용일 때만 확장된 인라인 안내와 재확인 동작을 제공한다. 같은 차단 이유를 status와 CTA 아래에서 중복 설명하지 않는다.
...
`/settings` route와 시스템·라이트·다크 테마 선택은 유지한다. 존재하지 않는 설정 항목을 만들지 않고, 외곽 카드보다 `화면 표시`라는 실제 preference group이 중심이 되도록 밀도를 조정한다. 구현 용어는 검증 가능한 운영 정보로서 보조 위치에 남기되 사용자의 결정과 결과를 설명하는 문구보다 앞서지 않는다.
...
- **readiness 표현 상태**: `checking`과 `ready`는 제목·상태명·필요 최소 정보로 구성한 compact status를 사용한다. `failed`는 API 상태 요청 자체를 확인하지 못한 상태, `unavailable`은 API가 응답했지만 worker가 작업을 받을 수 없는 상태로 구분해 확장 안내를 표시한다. 두 장애 상태는 재확인 동작을 유지한다.
- **readiness 정보 우선순위**: 마지막 확인 시각은 정상 상태의 주 콘텐츠가 아니며 보조 정보로 낮춘다. 제출 불가 이유는 readiness 영역과 CTA 인접 설명이 같은 긴 문장을 중복하지 않도록 하나를 canonical 안내로 삼고 다른 위치는 그 안내와 접근성 관계로 연결한다.
...
- **처리 방식 용어**: `속도 우선`과 `정확도 우선` 및 검증된 상대적 설명을 주 정보로 유지한다. `base.en`, `small.en`, 로컬 Whisper는 API 계약을 확인하는 보조 기술 정보로만 표시하며 제거하거나 값을 바꾸지 않는다.
...
- **readiness 상태**: `checking`·`ready`에서는 compact 표현, `failed`·`unavailable`에서는 확장 안내와 재확인 동작이 보이는지 검증한다. 상태 변화가 `status` 또는 `alert`로 전달되고, 같은 차단 설명이 화면에 중복 출력되지 않는지 확인한다.
...
- **수동 시각 확인**: 자동 테스트 뒤 light·dark의 390×844와 1280×900을 한 번의 bounded pass로 비교한다. 카드 중첩 감소, 첫 시각 초점, readiness 위계, 자막 CTA 거리, 설정 밀도를 함께 확인하고 발견 사항은 한 번에 수정한 뒤 최대 한 번만 재확인한다.
...
- 요청 내역 검색·필터·페이지네이션·전체 삭제·작업 취소·batch 요청.
...
- 근거는 `apps/web` PRODUCT 계약, 공통 DESIGN 계약, 2026-09-02 `apps/web` Impeccable critique(24/40), 현재 route·컴포넌트 구현, 기존 Web unit/browser 테스트다.
...
- 후속 티켓은 이 스펙 아래에서 별도 세션에 작성한다. 구현 순서는 `표면 계층 → compact readiness → 모바일 자막 밀도 → 삭제 undo → 설정·빈 상태·터치 영역 정리 → 통합 시각 검증`의 수직 slice를 권장한다.
